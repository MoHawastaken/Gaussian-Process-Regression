...
The class `GPC` solves binary classification problems by using a Gaussian process. It is initialized with training data $X$ ($D\times n$ matrix), target vector $y \in \{-1, 1\}^n$, the assumed noise level $\sigma$ and the vectorized covariance function `cov_func`. 

```{r, eval=FALSE}
GPC$new(X, y, noise, cov_func)
```
...
y <- rep(c(1,-1), each = n)
gaussian_classifier <- GPC$new(X, y, 1e-5, function(x,y) sqrexp(x,y,l = 1))
gaussian_classifier$plot()$plot
```
...
```{r eval=FALSE}
simulate_regression(func, limits, training_points, num_data = 10, 
                                noise = 0, error = function(x) 0, cov_names = names(cov_dict))

```

```{r eval=FALSE}
simulate_classification(func, training_points, limits, k, training_size = 10)
```
...
Simulating a random function with little noise, randomly drawn training data and covariance function squared exponential and its fit with `simulate_regression_gp` may look like this:

```{r eval=FALSE}
simulate_regression_gp(actual_cov, limits, error = function(x) 0, test_size = 300L, 
...
s <- simulate_regression_gp(cov_func(sqrexp, l = 1), limits = matrix(c(-5,5), nrow = 1), 
                         training_size = 10, random_training = TRUE, error = error_function(rnorm, sd = 0.01), show_pred = FALSE, noise = 0.01)
``` 
