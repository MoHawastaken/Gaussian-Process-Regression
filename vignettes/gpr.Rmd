...
simulate_regression(func, limits, training_points, num_data = 10, 
                                observation_noise = function(x) 0, cov_names = names(cov_dict), noise = 0)

```

```{r eval=FALSE}
simulate_classification(func, training_points, limits, training_size = 10, k = k)
```
...
```{r dpi=1000, out.width="100%", echo=FALSE, fig.width = 5, fig.asp = .62, message=FALSE, eval=TRUE}
s <- simulate_regression_gp(cov_func(sqrexp, l = 1), limits = matrix(c(-5,5), nrow = 1), training_size = 10, random_training = TRUE, observation_noise = iid_noise(rnorm, sd = 0.01), show_pred = FALSE, noise = 0.01)
``` 
