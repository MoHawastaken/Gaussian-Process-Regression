...
Typically one observes data sets $(x_i,y_i)$ with $y_i=f(x_i)+\varepsilon_i$ and assumes $\varepsilon \sim \norm{0,\sigma_n^2}$, $\sigma_n^2=\sigma^2\cdot I_n$.
Now given these data sets there are mostly two questions one likes to answer:
...
Gaussian$plot(seq(-10,10, by = 0.05))$plot
```
...
noise <- 0.5
y <- c(0.1*X^3 + rnorm(length(X), noise, 0))
...
```{r, eval=FALSE}
Gaussian_classifier <- GPC$new(X, y, noise, cov_func)
```
