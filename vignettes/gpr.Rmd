...
```{r eval = FALSE}
plot_posterior_draws(n = 5, testpoints = seq(min(self$X), max(self$X), length.out = 100))
```
...
y <- c(0.1*X^3 + rnorm(length(X), noise, 0))
Gaussian <- GPR.gammaexp$new(X, y, 0.1, 1, 1.5)
Gaussian$plot_posterior_variance(c(-2, 0, 1))
```
...
Decision regions are chosen with the threshold of 0.5, as the decison function maps into the intervall [0,1]. The labels of the points are also shown as 0 or 1, instead of -1 or 1. 
