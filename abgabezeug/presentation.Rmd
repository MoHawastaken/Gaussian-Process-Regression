...
ggplot2::ggplot(data = data.frame(x = X, y = y), ggplot2::aes(x = x, y = y, shape = 4)) +
                   ggplot2::scale_shape_identity() + ggplot2::theme_classic() + ggplot2::geom_point() + ggplot2::ylim(-2.2,2.2) + ggplot2::xlim(-10, 10)
```
...
3 Beobachtungen eines Gausschen Prozess' mit dieser Covarianzfunktion (l = 1)
...
Gaussian$plot_posterior_draws(5, c(-10, 10), 1000L)
```
...
g + ggplot2::ylim(-2.2,2.2) + ggplot2::xlim(-10, 10) + ggplot2::scale_y_continuous("y")
```
