...
- Stochastiser Prozess: $f:\Omega \times X \rightarrow Z$, $(\omega,x) \mapsto f_x(\omega)$ wobei $f_x(\omega)$ messbar in $\omega$ für alle $x\in X$.     
...
>- Wird charakterisiert über $m(x)=\mathbb{E}({f(x)})$ und $k(x,x')=\text{Cov}(f(x),f(x'))$. 

## Anwendung

1. Regression: Vorhersage des Prozesses zu anderen Zeitpunkten
+  Reale Anwendungen?
2. Klassifikation: Vorhersage der Zugehörigkeit von Beobachtungen zu verschiedenen Klassen
+  Reale Anwendungen?
...
- R6 Klasse
- Effizienz durch Speichern und Vektorisierung
...
Daten werden mit Fehler $\varepsilon$ beobachtet, $y_i = f(x_i) + \varepsilon_i$ $\varepsilon_i \overset{\text{iid}}{\sim} \mathcal{N}(0, \sigma_n)$


## predict Algorithmus
Daten werden mit Fehler $\varepsilon$ beobachtet, $y_i = f(x_i) + \varepsilon_i$ $\varepsilon_i \overset{\text{iid}}{\sim} \mathcal{N}(0, \sigma_n^2)$
...
return: $\bar{f}(X_*)$, $\overline{\text{V}}(f(X_*))$
...
Ziel: Suchen Prozess, aus dem die beobachteten Daten am Wahrscheinlichsten entstehen

Idee: Bestimmung einer passenden Kovarianzfunktion

Methode:

- Maximierung der Marginal Log-Likelihood für verschiedene Kovarianzfunktionen und Parameter
- Nutzen Newton Methode, `optim()`

Marginal Log-Likelihood: *Gute Erklärung fehlt*
...
- Unterschiedliche Maximierungsmethoden für verschiedene Kovarianzfunktionen

Feature: Übergibt man Regression eine Liste von Kovarianzfunktionen, wird fit automatisch ausgeführt.

Achtung: Funktioniert nur über eingespeicherte Kovarianzfunktionen
...
```{r dpi=k, out.width="95%", echo=FALSE, fig.width = 5, fig.asp = .62, message=TRUE}
X <- matrix(seq(-5,5,by = 0.5), nrow = 1)
noise <- 0.5
y <- c(0.1*X^3 + rnorm(length(X), 0, noise))
...
## Klassifikation mit GP

- Binäres Klassifikationsproblem $g: X \rightarrow \{-1, +1\}$
- Modellierung der posterior Wahrscheinlichkeit $\mathbb{P}(g = +1|x)$ durch $\sigma(f(x))$
- $f(x)$ ist ein Gaußscher Prozess
- $\sigma$ ist die Sigmoid Funktion $\sigma : \mathbb{R} \rightarrow [0,1]$, $z \mapsto \frac{1}{1 + \exp(-z)}$
