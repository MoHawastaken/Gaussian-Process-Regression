...
Wir haben uns dazu entschieden, eine R6-Klasse für die Regression anzulegen. Grund hierfür war die leichte Erstellung von schreibgeschützen Bereichen. Dies erschien sinnvoll, da die Algorithmen einige rechenaufwendige Schritte beinhalten, die nur einmal ausgeführt werden sollten (z.B. Cholesky Zerlegung) und somit beim Erstellen der Klasse ausgeführt werden müssen, wir aber den Nutzer davor schützen wollten beim Hinzufügen von Daten eine neue Berechnung der implizit genutzten Variablen in den Algorithmen zu vergessen. Obwohl unsere Klasse die Eingabe beliebiger Daten und Kovarianzfunktionen ermöglicht, besticht sie durch einfache und optimierte Nutzung, indem man nur die nötigen Inputs X, y und noise angibt. Eine Vektorisierung der Kovarianz -und Vorhersage-Funktionen hat die Effizienz dieser Methode und damit auch der Plots erheblich verbessert.

Die Funktion fit(X ,y, noise, cov_names) bestimmt die optimale Kovarianzfunktion unter den in cov_names angegebenen durch Maximierung der log likelihood der a posteriori Verteilung des zugrundeliegenden Gaußschen Prozesses als Funktion der Hyperparameter. Dieses Verfahren ist erfordert weniger Rechenleistung als zum Beispiel ein Cross-Validation-Verfahren. Hier haben wir Metaprogrammierung genutzt, um Kovarianzfunktionen mit unterschiedlich vielen Parametern zu optimieren, außerdem mit error handlern die Fehleranfälligkeit der Choleskyzerlegung von möglicherweise singulären Matrizen abgefangen, sodass für jede Kovarianzfunktion der optimale Parameter bis zum ersten Fehler zurückgegeben wird. So erzeugt fit stabile Ergebnisse, vor allem für noise > 0.

Unsere Visualisierungsfunktionen sprechen für sich. Die Dokumentationen von GPR und fit erläutern die einfache Nutzung unseres Pakets mit Minimalbeispielen.
